...
const crypto = require('crypto'),
    ...
    path = require('path'),
    // notificationModel = require(path.resolve('.') + '/models/mongodb').notification,
    // distributionlistModel = require(path.resolve('.') + '/models/mongodb').distributionlist,
...
let Utils = function () {
    ...
    this.generateJWTToken = (user) => {
        return new Promise((resolve, reject) => {
            try {
                // To generate token
                let expires = expiresIn(config.environment.redis.passwordExpiry);
    ...
    this.getTokenDetail = (authHeader) => {
        return new Promise((resolve, reject) => {
            ...
            if (authHeader) {
                ...
                token = authHeader.length>1?authHeader[1]:authHeader[0];
                if (!token || token === undefined) {
                    reject(new Error("Missing token in header"));
                    return;
                }
